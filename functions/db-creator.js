...
async function createTable(conn, createTableQuery, tableName) {
  console.log(`Checking or creating ${tableName}...`);
  await conn.query(createTableQuery);
}

// Function to create an event
async function createEvent(conn, createEventQuery) {
  console.log('Checking or creating event...');
  await conn.query(createEventQuery);
}
...
async function setupDatabase() {
  ...
  try {
    ...
    for (const { query, name } of tableQueries) {
      await createTable(conn, query, name);
    }

    // Create the delete old song requests event
    const createDeleteOldSongRequestsEventQuery = `
      CREATE EVENT IF NOT EXISTS \`delete_old_song_requests\`
        ON SCHEDULE EVERY 30 MINUTE STARTS '2024-08-29 10:09:55'
        ON COMPLETION PRESERVE
        ENABLE
        COMMENT ''
        DO DELETE FROM \`song_requests\` WHERE \`created_at\` < NOW() - INTERVAL 1 MINUTE
    `;
    await createEvent(conn, createDeleteOldSongRequestsEventQuery);
